...
const Register = () => {
  ...
  const location = useLocation(); // Replace location prop with useLocation hook
  ...
  useEffect(() => {
    if (success) {
      setTimeout(() => {
        navigate(routes.EMAIL_VERIFICATION + `?redirect=${redirect}`); // Use navigate for redirection
      }, 5000);
    }
    ...
    <FormContainer>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Button type="submit" variant="contained" color="primary" fullWidth onClick={handleSubmit} disabled={loading}>
          {loading ? <CircularProgress color="inherit" sx={{ color: '#fff' }} /> : <>Register</>}
        </Button>
      </Form>
