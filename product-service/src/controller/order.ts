...
export const getOrders = asyncHandler(async (req, res, _) => {
  res.status(200).send(res.advanceResults);
});

export const authOrder = asyncHandler(async (req, res, _) => {
...
export const getOrder = asyncHandler(async (req, res, _) => {
...
export const payment = asyncHandler(async (req, res, next) => {
  ...
  order.isPaid = true;
  order.paidAt = new Date(Date.now()) as NativeDate;
  if (req.body.status) {
    order.paymentResult = {
      id: req.body.id,
    ...
    order.paymentResult = {
      ...
      status: "200",
      update_time: new Date(Date.now()) as NativeDate,
...
export const deliverOrder = asyncHandler(async (req, res, next) => {
  ...
  order.deliveredAt = new Date(Date.now()) as NativeDate;
